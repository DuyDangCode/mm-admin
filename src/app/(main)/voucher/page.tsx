    ...
    <ScrollArea className='h-full w-full z-[0]' py='1rem' px='2rem'>
      <Group justify='space-between'>
        ...
        <Button
          ...
          onClick={() => router.push(`${currentPath}/create_voucher`)}
        >
          Tạo voucher mới
        </Button>
      </Group>
              ...
              <Table.Tbody>
                {vouchers.data?.map((voucher) => (
                  <Table.Tr key={`row-${voucher._id}`}>
                    ...
                    <Table.Td>
                      {dayjs(voucher.discount_start_date).format('DD/MM/YYYY')}
                    </Table.Td>
                    <Table.Td>
                      {dayjs(voucher.discount_end_date).format('DD/MM/YYYY')}
                    </Table.Td>
                    <Table.Td>{voucher.discount_apply_to}</Table.Td>
                    <Table.Td>
                      {voucher.discount_is_active ? 'ACTIVE' : 'INACTIVE'}
                    </Table.Td>
                    <Table.Td className='cursor-pointer'>
                      <Text
                        onClick={() =>
                          router.push(`${currentPath}/${voucher._id}`)
                        }
                        c='turquoise'
                      >
                        Xem

...
const VoucherDetail = ({
  ...
  const voucherId = use(params).voucher_id
  ...
  const products = useQuery({
    queryKey: ['products_voucher', voucherId],
    queryFn: () => {
      const voucherService = new VoucherService(user)
      return voucherService.getProductOfVoucher(voucherId)
    },
    enabled: !!user,
  })

  const deleteVoucherMutation = useMutation({
    mutationKey: ['deleteVoucher', voucherId],
  ...
  return (
    <>
      {voucher.isPending || deleteVoucherMutation.isPending ? (
        <SkeletonLoading />
      ) : (
        <>
          {edit ? (
              ...
              <div className='flex flex-col gap-[24px] py-[16px] px-[16px] '>
                ...
                <Group justify='space-between' wrap='nowrap' gap='0'>
                  <VoucherInfoField label='Hình thức'>
                    {voucher.data?.discount_type == 'percent'
                      ? 'Giảm theo %'
                      : 'Giảm số tiền nhất định'}
                  </VoucherInfoField>
                  <VoucherInfoField label='Ngày bắt đầu'>
                    {dayjs(voucher.data?.discount_start_date).format(
                      'DD/MM/YYYY',
                    )}
                  </VoucherInfoField>
                  <VoucherInfoField label='Ngày kết thúc'>
                    {dayjs(voucher.data?.discount_end_date).format(
                      'DD/MM/YYYY',
                    )}
                  </VoucherInfoField>
                </Group>
                ...
                <Group wrap='nowrap' gap='0'>
                  ...
                  <VoucherInfoField label='Áp dụng cho'>
                    {voucher.data?.discount_apply_to == 'all'
                      ? 'Tất cả'
                      : 'Một số sản phẩm nhất định'}
                  </VoucherInfoField>
                </Group>
                {voucher.data?.discount_products &&
                  voucher.data?.discount_products.length > 0 && (
                    <div className='flex flex-col border-[0.5px] border-solid rounded-[4px] w-full py-[12px] px-[16px]'>
                      ...
                      >
                        ...
                        {voucher.data.discount_apply_to == 'specific' &&
                          products.isSuccess && (
                            <Table.Tbody>

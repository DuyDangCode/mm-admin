...
const UpdateVoucher = ({ voucher, back }: { voucher: any; back: any }) => {
  ...
  const [type, setType] = useState<string | undefined | null>(
    voucher.discount_type == 'percent'
      ? 'Giảm theo %'
      : 'Giảm số tiền nhất định',
  )
  const [apply, setApply] = useState<string | undefined | null>(
    voucher.discount_apply_to == 'all'
      ? 'Toàn bộ sản phẩm'
  ...
  const form = useForm({
    ...
    validate: {
      ...
      description: (value) =>
        value.length === 0 ? 'Vui lòng nhập miêu tả' : null,
      code: (value) => (value.length === 0 ? 'Vui lòng nhập code' : null),
      max_uses: (value) =>
        value === null ? 'Vui lòng nhập số lần sử dụng tối đa' : null,
      max_uses_per_user: (value) =>
        value === null
          ? 'Vui lòng nhập số lần sử dụng tối đa của mỗi user'
          : null,
      min_order_value: (value) =>
        value === null ? 'Vui lòng nhập giá trị đơn hàng tối thiểu' : null,
      end_date: (value) =>
        value === null ? 'Vui lòng nhập ngày kết thúc' : null,
    },
  })
  ...
  const handleUpdateVoucher = (formData: any) => {
    ...
    // })
    if (type == '' || apply == '') toast.error('Vui lòng nhập đủ thông tin')
    else
      updateVoucherMutation.mutate({
        ...
        discount_end_date: dayjs(formData.end_date).format(
          'YYYY-MM-DD HH:mm:ss',
        ),
        ...
        discount_apply_to:
          apply == 'Những sản phẩm nhất định' ? 'specific' : 'all',
  ...
  return (
    <>
      {updateVoucherMutation.isPending ? (
        ...
        <form
          onSubmit={form.onSubmit(handleUpdateVoucher)}
          id='updateVoucherForm'
        >
                    ...
                    addedProduct.map((item) => (
                      <Stack align='center'>
                        ...
                        <ActionIcon
                          ...
                          onClick={() => {
                            setAddedProduct(
                              addedProduct.filter((p) => p._id != item._id),
                            )
                          }}
                        >
                          <IconTrash />

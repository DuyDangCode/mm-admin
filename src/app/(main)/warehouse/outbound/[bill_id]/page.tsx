...
export default function ExportBillView({
  params,
}: {
  ...
  const billId = use(params).bill_id

  const bill = useQuery({
    queryKey: ['outbound_bill', billId],
    queryFn: () => {
      const billService = new BillService(user)
      return billService.getExportById(billId)
    },
    enabled: !!user,
  })
        ...
        <div className='flex flex-col gap-[24px] py-[16px] px-[16px] '>
          ...
          <Group justify='space-between' wrap='nowrap' gap='0'>
            <BillInfoField label='Ngày tạo'>
              {dayjs(bill.data?.bill_info.bill_date.toString())
                .format('DD/MM/YYYY')
                .toString()}
            </BillInfoField>
          ...
          <div className='flex flex-col border-[0.5px] border-solid rounded-[4px] w-full py-[12px] px-[16px]'>
            ...
            >
              ...
              <Table.Tbody>
                {bill.data?.products_info
                  .slice(activePage * 5 - 5, activePage * 5 - 1)
                  .map((item) => (
